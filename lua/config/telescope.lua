...
-- 共通の除外定義
local ignore_dirs  = {
    ".git/", "node_modules/", "dist/", "build/", ".venv/", "venv/", "coverage/", "target/",
}
...
local function rg_globs()
    ...
    for _, d in ipairs(ignore_dirs) do
        table.insert(args, "--glob"); table.insert(args, "!" .. "**/" .. d .. "**")
    end
    for _, f in ipairs(ignore_files) do
        table.insert(args, "--glob"); table.insert(args, "!" .. "**/" .. f)
    end
    for _, e in ipairs(ignore_exts) do
        table.insert(args, "--glob"); table.insert(args, "!" .. "**/*." .. e)
    end
    return args
end
...
local function fd_excludes()
    local args = {}
    for _, d in ipairs(ignore_dirs) do table.insert(args, "--exclude=" .. d) end
    for _, f in ipairs(ignore_files) do table.insert(args, "--exclude=" .. f) end
    for _, e in ipairs(ignore_exts) do table.insert(args, "--exclude=*." .. e) end
    return args
end
